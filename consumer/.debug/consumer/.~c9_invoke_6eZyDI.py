...
def lambda_handler(event, context):
    ...
    receipt_handle  = event['fang'][0]['receiptHandle'] #sqs message
    #'eventSourceARN': 'arn:aws:sqs:us-east-1:561744971673:producer'
    event_source_arn = event['fang'][0]['eventSourceARN']

    names = [] #Captured from Queue

    # Process Queue
    for record in event['fang']:
    ...
    # Write result to S3
    write_s3(df=df, bucket="fangsentiment69", name=names)